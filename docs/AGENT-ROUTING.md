...
**Solution** : un classifier déterministe qui s'exécute dans le hook `UserPromptSubmit`, lit `config/agent-routing.json`, et injecte un message — impératif (`force: true`) ou suggestif (`force: false`) — avant que Claude ne traite le prompt.
...
Fichier : `config/agent-routing.json` (créé depuis `.example` à l'install).
...
1. Éditer `~/.claude/config/agent-routing.json`
2. Ajouter une entrée dans le tableau `rules`
3. Tester : `echo '{"prompt":"<exemple>"}' | bash ~/.claude/scripts/suggest-skill.sh`

## Règles par défaut

Voir `config/agent-routing.json.example` :
